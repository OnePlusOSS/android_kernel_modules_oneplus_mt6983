	...
	IN struct SW_RFB *prSwRfb, IN uint32_t u4RxVector0)
{
	...
	if (rate.preamble == LLS_MODE_OFDM) {
		...
		/* To avoid too much logs when Rx VHT CBW160-MCS11 PPDUs*/
		if (rate.bw >= STATS_LLS_MAX_VHT_BW_NUM)
			rate.bw = STATS_LLS_MAX_VHT_BW_NUM - 1;
		if (rate.rateMcsIdx >= STATS_LLS_VHT_NUM)
			rate.rateMcsIdx = STATS_LLS_VHT_NUM - 1;
