		...
		while (kmsg_dump_get_line(&kmsg_dumper, true, line_buf, BUF_SIZE_OF_LINE, &len))
		{
			...
			if (isfull || isforcewb) {
				...
				if (likely(ret > 0)) {
					...
					oprkl_err_print("generic_write_checks failed %d\n", ret);
					inode_unlock(inode);
					return 1;
				}
				inode_unlock(inode);
				if (likely(ret > 0)) {
					...
					if (ret < 0) {
						oprkl_err_print("Write sync failed %d\n", ret);
						return 1;
					}
				} else {
					oprkl_err_print("generic_perform_write failed %d\n", ret);
					return 1;
				}
				if(isfull) {
					write_block_cnt ++;
...
int init_header(struct block_device *bdev, struct kernel_log_thread_header *head)
{
	...
	if (memcmp(head->magic, LOG_THREAD_MAGIC, strlen(LOG_THREAD_MAGIC)) == 0) {
		...
		memset(head, 0, sizeof(struct kernel_log_thread_header));
		max_size = (strlen(LOG_THREAD_MAGIC) > (sizeof(head->magic)-1)) ? (sizeof(head->magic)-1) : strlen(LOG_THREAD_MAGIC);
		memcpy(head->magic, LOG_THREAD_MAGIC, max_size);
	}
...
static int reserve_log_main(void *arg)
{
	...
	wb_kernel_log(bdev, data_buf, line_buf);
	if (phx_is_system_server_init_start())
	{
