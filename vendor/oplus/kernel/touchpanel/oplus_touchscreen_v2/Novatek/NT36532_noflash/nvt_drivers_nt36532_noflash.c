...
*/
	...
	chip_info->p_nvt_test_para->doze_x_channel					= chip_info->doze_x_num;
	...
	chip_info->p_nvt_test_para->config_lmt_lpwg_diff_n			= 0;
	chip_info->p_nvt_test_para->fdm_x_channel					 = chip_info->doze_x_num;
	/*if (ts->chip_type_cascade) {
...
static void nvt_prase_dts(struct device *dev, void *chip_data)
{
	...
	TPD_INFO("chip_info->doze_x_num = %d\n", chip_info->doze_x_num);
}
