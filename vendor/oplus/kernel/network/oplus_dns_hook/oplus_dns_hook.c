...
static int parse_querys(char *start, u32 payload_len, int *querys_len, query_info_t *queryList,
...
static int dns_hook_process_postrouting(struct sk_buff *skb, int hook, const struct nf_hook_state *state)
{
	...
	ret = parse_querys((char *)(dnshdr + 1), payload_len, &query_len, query_list, query_count, &match_count, hook);
    if (ret) {
        kfree(query_list);
        return -1;
    }
