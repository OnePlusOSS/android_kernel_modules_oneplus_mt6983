...
/* modify sia max owi node. */
#define MAX_OWI_MODE						(32)
...
/* Add for bring up audio */
static DEFINE_MUTEX(g_en_dyn_product_mutex);
static int g_en_dyn_product =0;
...
static const char *support_chip_type_name_table[] = {
	...
	/* compatible sia8102 and sia8102s. */
	[CHIP_TYPE_SIA8102S]  = "sia8102s",
...
void distinguish_chip_type(sipa_dev_t *si_pa)
{
	...
	if (CHIP_TYPE_UNKNOWN == si_pa->chip_type) {
		...
		pr_info("[ info][%s] %s: there is no si_pa device \r\n",
	        ...
	        /* compatible sia8102 and sia8102s. */
		if ((1 == si_pa->en_dyn_id) && (1 == si_pa->en_dyn_product)) {
			if (1 == check_sipa_status(si_pa)) {
				si_pa->chip_type = CHIP_TYPE_SIA8102S;
...
static void sia81xx_pa_enable_l(int enable, int mode)
{
	...
	/*Add for bring up audio */
	mutex_lock(&g_en_dyn_product_mutex);
	if(1 == g_en_dyn_product){
	    sia81xx_pa_enable_by_scene(enable, mode, SIA81XX_CHANNEL_L2);
	}
	mutex_unlock(&g_en_dyn_product_mutex);
}
...
static int sipa_probe(struct platform_device *pdev)
{
	...
	struct pinctrl_state *pinctrl_state = NULL;
        /* Add for bring up audio */
	int disable_pin = 0, rst_pin = 0, id_pin = 0, en_dyn_id = 0 ,en_dyn_product =0;
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_PA_MANAGER)
	struct oplus_speaker_device *speaker_device = NULL;
	bool new_speaker_device_node = false;
#endif /* CONFIG_SND_SOC_OPLUS_PA_MANAGER */
...
#endif
		/* Add for bring up audio */
		ret = of_property_read_u32(pdev->dev.of_node, "en_dynamic_product", &en_dyn_product);
		if ((0 != ret) || (1 != en_dyn_product)) {
			pr_debug("[debug][%s] %s: en_dyn_product != 1 ,use default vaule \r\n", LOG_FLAG, __func__);
			en_dyn_product = 0;
			mutex_lock(&g_en_dyn_product_mutex);
			g_en_dyn_product =0;
			mutex_unlock(&g_en_dyn_product_mutex);
		}
		si_pa->en_dyn_product = (unsigned int)en_dyn_product;
		mutex_lock(&g_en_dyn_product_mutex);
		g_en_dyn_product =(unsigned int)en_dyn_product;
		mutex_unlock(&g_en_dyn_product_mutex);
	...
	if (!si_pa->sia91xx_wq)
		return -ENOMEM;
	/* compatible sia8102s and sia8102. */
	if (CHIP_TYPE_SIA8100X == si_pa->chip_type ||
		CHIP_TYPE_SIA8102S == si_pa->chip_type  ||
	...
	if(1 == si_pa->en_dyn_id){
		...
		pr_info("%s():,oplus_register start\r\n", LOG_FLAG, __func__);
		/* Add for bring up audio */
		pr_info("%s():,en_dyn_product =%u \r\n", LOG_FLAG, __func__,si_pa->en_dyn_product);
		if(1 == si_pa->en_dyn_product) {
			if(speaker_device == NULL) {
				goto pa_register;
                        }
	        }else{
...
pa_register:
				...
				if ((si_pa->channel_num == 0 || si_pa->channel_num == 2) && (speaker_device != NULL)) {
					speaker_device->chipset = MFR_SI;
					speaker_device->type = L_SPK;
					speaker_device->vdd_need = 0;
					speaker_device->speaker_enable_set = sia81xx_pa_enable_l;
				...
				} else if ((si_pa->channel_num == 1 || si_pa->channel_num == 3) && (speaker_device != NULL)) {
					speaker_device->chipset = MFR_SI;
					speaker_device->type = R_SPK;
					speaker_device->vdd_need = 0;
					speaker_device->speaker_enable_set = sia81xx_pa_enable_r;
