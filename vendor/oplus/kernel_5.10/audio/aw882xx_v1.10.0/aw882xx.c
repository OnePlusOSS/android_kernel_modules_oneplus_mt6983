				...
				loff_t *ppos)
{
	...
	if (ret <= 0) {
		...
		for (i = 0; i < ret; i++) {
			len += snprintf((char *)&buf+len, buf_size-len, "%d,", cali_re[i]);
		}
		len += snprintf((char *)&buf+len-1, buf_size-len, "\0"); /* -1 delete the last "," */
	}
...
/* copy calibrated pa count to user space */
				...
				loff_t *ppos)
{
	...
	len += snprintf(buf+len, buf_size-len, "%d", cali_pa_cnt);
	len += snprintf(buf+len, buf_size-len, "\0");
...
/* write calibrated re to driver from user space persist nv */
...
/* copy calibrated f0 to user space */
				...
				loff_t *ppos)
{
	...
	if (ret <= 0) {
		...
		for (i = 0; i < ret; i++) {
			len += snprintf((char *)&buf+len, buf_size-len, "%d,", cali_f0[i]);
		}
		len += snprintf((char *)&buf+len-1, buf_size-len, "\0"); /* -1 delete the last "," */
	}
...
/* write calibrated f0 to driver from user space persist nv */
